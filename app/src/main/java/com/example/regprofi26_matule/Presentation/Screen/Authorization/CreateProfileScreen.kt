...
@Composable
fun CreateProfileScreen(navController: NavHostController, viewModel: AuthViewModel){
    ...
    Column(modifier = Modifier.fillMaxWidth().padding(horizontal = 20.dp)) {
        ...
        Text("Создание профиля", style = createMatuleTypography().title1Heavy)
        ...
        Text("В профиле будут храниться результаты проектов и ваши описания.",
            ...
            color = MatuleTheme.colors.description)
        SpacerH(32)
        LazyColumn(modifier = Modifier.fillMaxWidth().padding(bottom = 90.dp ),
            ...
            item {
                Inputs(
                    state.patronymic, "Отчество",
                    {
                        viewModel.updateState(state.copy(patronymic = it))
                    })
            }
            item {
                Inputs(
                    state.surname, "Фамилия",
                    {
            ...
            item {
                SelectDate(
                    state.dateUser, "Дата рождения",
                    {
                        viewModel.updateState(state.copy(dateUser = it))
                    }
                )
            }
            item {
